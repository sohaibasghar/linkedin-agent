...
export async function POST(request: NextRequest) {
  ...
  if (!postId || !action) {
    return NextResponse.json(
      { error: 'BAD_REQUEST', message: 'postId and action are required' },
      { status: 400 }
    );
  }
  ...
  if (action === 'reject') {
    ...
    if (!post) {
      return NextResponse.json(
        { error: 'POST_NOT_FOUND', message: `No draft post found with id: ${postId}` },
        { status: 404 }
      );
    }
      ...
      return NextResponse.json(
        {
          ...
          message: `Post ${postId} is in status ${post.status}; approve/reject only allowed on DRAFT`,
